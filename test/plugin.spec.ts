...
describe('Plugin', () => {
  ...
  describe('Configuration', () => {
    it('hasHandler', () => {
      ...
      const test = new Plugin(config, { serverless: true });
  ...
  describe('Configuration', () => {
    it('hasFile', () => {
      ...
      expect(test.file).toEqual('/foo/bar.toml');
    });
  });

  describe('Configuration with array of files', () => {
    it('hasFile', () => {
      const config = {
        cli: { log: () => null },
        config: {
        ...
        service: {
          custom: {
            output: {
              file: ['foo/bar.toml', 'foo/bar.json'],
            },
          },
          provider: {
            name: 'aws',
      ...
      expect(test.file).toEqual(['/foo/bar.toml', '/foo/bar.json']);
    });
  });
});
