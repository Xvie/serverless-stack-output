...
export default class StackOutputPlugin {
  ...
  get handler() {
    return this.getConfig('handler') as string;
  }
  ...
  private format(value: string) {
    return util.format('%s/%s', this.serverless.config.servicePath, value);
  }

  private getConfig(key: string) {
    const configValue = this.output[key];
    console.log('configValue', configValue);
    if (Array.isArray(configValue)) {
      return configValue.map((value: string) => this.format(value));
    }
    return this.format(configValue);
  }
